...
import { Create, Update } from '../services/Action.service';
...
export const ActionForm: React.FC<ActionFormProps> = ({ initialValues, onSubmit, onCancel }) => {
    ...
    const handleSubmit = async (formEvent: React.FormEvent) => {
        ...
        try {
            ...
            const validatedFormData = ActionFormSchema.parse(formData);
    ...
    return (
        <Styled.Form onSubmit={handleSubmit}>
            <Styled.FieldWrapper>
                <Styled.Label htmlFor="nome"> Nome do tipo de ação </Styled.Label>
            ...
            <Styled.Footer>
                ...
                <Styled.FooterButton type="submit" $variant="primary" $formType="action"> {initialValues?.id ? 'Atualizar tipo de ação' : 'Cadastrar tipo de ação'} </Styled.FooterButton>
            </Styled.Footer>
        </Styled.Form>
    );
};